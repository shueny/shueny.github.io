...
export function makeStaticProps(
  ...
  return async function getStaticProps(ctx: StaticPropsParam) {
    const i18nProps = await await getI18nProps(ctx, source);
...
export function makeISRStaticPaths(
  fetchResource: (pathSlugs: PathSlug[]) => Promise<unknown>
) {
  return async function getStaticPaths() {
    const paths = await fetchResource(getPathSlugs());
    return {
      paths,
      fallback: "blocking",
    };
...
const readSourceFiles = (folders: string[], locale: LANGS) => {
  const basePath = path.join("./src/mock", locale);
