    ...
    projects: {
      ...
      data: [
        ...
        },
        {
          ...
          description:
            'An animated 3D explainer of Retrieval-Augmented Generation (RAG), the technique that lets an LLM answer from your own documents instead of its training memory. One question, "How long is our refund window?", travels through all six steps, from chunking to a cited answer, compared against an LLM guessing without RAG. Bilingual (EN / 中文) and fully offline.',
          problem:
            "What RAG is. A large language model answers from what it memorised during training. That knowledge is frozen at a cut-off date and has never seen your company's private documents, so when you ask about an internal policy it does not say \"I don't know\". It produces a fluent, confident guess.\n\nRetrieval-Augmented Generation fixes this without retraining the model. At question time the system first retrieves the most relevant passages from your own documents, then hands them to the model together with the question. The model answers from that evidence and cites where it came from.\n\nWhy it is hard to explain. The idea is simple, but the vocabulary is not: embeddings, vector databases, top-K, cosine similarity. Most explanations are either a box-and-arrow diagram or a wall of jargon. Stakeholders nod along and still expect the model to \"just know\" company policy, because they never see where an answer actually comes from.",
          solution:
            'I turned the pipeline into a story around one concrete question: "How long is our refund window?"\n\nIt opens with the failure case. Without RAG, the model answers "usually 30 days" with no source: a plausible hallucination. Then the same question travels through six chapters:\n1. Prepare: company documents are split into chunks with a small overlap, and every chunk keeps its source.\n2. Embed: an embedding model turns each chunk into a vector, its coordinates on a semantic map.\n3. Ask: the question is embedded with the same model.\n4. Retrieve: the question vector is compared with every chunk; the three closest win, with their similarity scores.\n5. Augment: the question and those three chunks become one prompt.\n6. Generate: the model answers from the chunks and cites Refund Policy v3 §2.\n\nIt closes with the three safeguards that keep retrieval reliable in production: hybrid search, reranking and citations.',
          techDeepDive:
            'RAG runs in two phases.\n\nIndexing (offline, whenever documents change): extract plain text from PDFs, wikis and databases; split it along paragraph boundaries into 300–500 character chunks, with a small overlap so no sentence is cut in half; attach source metadata (document, section) to every chunk; run each chunk through a pre-trained embedding model, which outputs a vector (1,536 numbers in the example) in which similar meaning lands close together; store the vector, the original text and the source together in a vector database.\n\nQuery time (every question): embed the question with the same model; rank chunks by cosine similarity, where 1 means the same meaning and 0 means unrelated (large collections use an approximate-nearest-neighbour index instead of comparing every chunk); keep the top K (here K = 3, scores 0.92 / 0.81 / 0.77); build an augmented prompt that tells the model to answer only from the provided context and to cite it; generate. Retrieval picks the relevant chunks, not the answer. The LLM still writes the answer, but now from evidence.\n\nWhere it goes wrong, and the fixes: bad chunking can split a key fact, and vague questions retrieve the wrong chunks. Hybrid search adds keyword matching to vector similarity, so product names, model numbers and clause IDs are not missed. Reranking retrieves a wider set (for example the top 20) and lets a second model read each chunk closely and re-order them. Citations let users and reviewers check the original.\n\nWhen to use it, and when not: RAG earns its complexity when the answer lives in private documents the model has never seen, when that information changes often, when answers need citations people can check, or when the material is too large to fit in one prompt. It is overkill for general knowledge the model already has, for a source of a few pages that can simply go into the prompt, for structured data (orders, stock), where a SQL query or an API call is more precise, and for rewriting, translating or changing tone, which involve no retrieval at all. Pilotfit applies the same idea: when it matches a job to a user\'s experiences, 60% of the score is embedding similarity and 40% is exact tech-tag overlap, so a specific technology is never left to vector distance alone.\n\nHow the explainer is built: a single 3:12 timeline drives a 3D scene (document stack, embedding ring, vector-database sphere, the user and the LLM planet), with HTML overlays for each close-up chapter so the text stays sharp and translatable. Every chapter reuses the same refund-policy example, so the numbers stay consistent end to end. Each language ships as one self-contained HTML file with scripts, styles and fonts inlined.',
          features: [
            'What RAG is, and why an LLM hallucinates without it, shown before any jargon',
            'When RAG is worth it, and when it is overkill',
            'Six-chapter pipeline: Prepare → Embed → Ask → Retrieve → Augment → Generate',
            'Chunking close-up: paragraph splits, overlap and source tags',
            'Semantic map and cosine-similarity ranking with a visible top-K = 3',
            'Production safeguards: hybrid search, reranking and citations',
    ...
    projects: {
      ...
      data: [
        ...
        },
        {
          ...
          description:
            'Eine animierte 3D-Erklärung von Retrieval-Augmented Generation (RAG), der Technik, mit der ein LLM aus den eigenen Dokumenten antwortet statt aus seinem Trainingsgedächtnis. Eine Frage („Wie lange ist unser Rückgabefenster?“) durchläuft alle sechs Schritte, vom Aufteilen der Dokumente bis zur Antwort mit Quelle, im Vergleich zu einem LLM, das ohne RAG rät. Zweisprachig (EN / 中文) und komplett offline.',
          problem:
            'Was RAG ist. Ein großes Sprachmodell antwortet aus dem, was es im Training gelernt hat. Dieses Wissen endet an einem Stichtag und hat die internen Dokumente eines Unternehmens nie gesehen. Fragt man nach einer internen Richtlinie, sagt es nicht „weiß ich nicht“, sondern liefert eine flüssige, selbstsichere Vermutung.\n\nRetrieval-Augmented Generation löst das, ohne das Modell neu zu trainieren. Zum Zeitpunkt der Frage sucht das System zuerst die relevantesten Abschnitte aus den eigenen Dokumenten heraus und gibt sie zusammen mit der Frage an das Modell. Das Modell antwortet auf Basis dieser Belege und nennt die Quelle.\n\nWarum das schwer zu erklären ist. Die Idee ist einfach, das Vokabular nicht: Embeddings, Vektordatenbanken, Top-K, Kosinus-Ähnlichkeit. Die meisten Erklärungen sind entweder ein Kästchen-und-Pfeile-Diagramm oder eine Wand aus Fachbegriffen. Stakeholder nicken und erwarten trotzdem, dass das Modell die Richtlinien „einfach weiß“, weil sie nie sehen, woher eine Antwort kommt.',
          solution:
            'Ich habe die Pipeline als Geschichte rund um eine konkrete Frage erzählt: „Wie lange ist unser Rückgabefenster?“\n\nSie beginnt mit dem Fehlerfall: Ohne RAG antwortet das Modell „meist 30 Tage“, ohne Quelle, eine plausible Halluzination. Dann durchläuft dieselbe Frage sechs Kapitel:\n1. Vorbereiten: Firmendokumente werden in Abschnitte mit kleiner Überlappung geteilt, jeder Abschnitt behält seine Quelle.\n2. Einbetten: Ein Embedding-Modell macht aus jedem Abschnitt einen Vektor, seine Koordinaten auf einer semantischen Karte.\n3. Fragen: Die Frage wird mit demselben Modell eingebettet.\n4. Abrufen: Der Fragevektor wird mit allen Abschnitten verglichen; die drei nächsten gewinnen, mit ihren Ähnlichkeitswerten.\n5. Anreichern: Frage und diese drei Abschnitte werden zu einem Prompt.\n6. Generieren: Das Modell antwortet aus den Abschnitten und zitiert Rückgaberichtlinie v3 §2.\n\nZum Schluss die drei Schutzmechanismen, die das Abrufen in der Praxis zuverlässig machen: hybride Suche, Reranking und Quellenangaben.',
          techDeepDive:
            'RAG läuft in zwei Phasen.\n\nIndexierung (offline, bei jeder Dokumentänderung): Klartext aus PDFs, Wikis und Datenbanken extrahieren; entlang von Absätzen in Abschnitte von 300–500 Zeichen teilen, mit kleiner Überlappung, damit kein Satz zerschnitten wird; jedem Abschnitt Quellen-Metadaten (Dokument, Abschnitt) anhängen; jeden Abschnitt durch ein vortrainiertes Embedding-Modell schicken, das einen Vektor ausgibt (im Beispiel 1.536 Zahlen), in dem ähnliche Bedeutung nah beieinander liegt; Vektor, Originaltext und Quelle gemeinsam in einer Vektordatenbank speichern.\n\nAbfrage (bei jeder Frage): die Frage mit demselben Modell einbetten; Abschnitte nach Kosinus-Ähnlichkeit sortieren, wobei 1 gleiche Bedeutung und 0 keinen Bezug heißt (große Sammlungen nutzen einen Approximate-Nearest-Neighbour-Index, statt jeden Abschnitt zu vergleichen); die Top K behalten (hier K = 3, Werte 0,92 / 0,81 / 0,77); einen angereicherten Prompt bauen, der das Modell anweist, nur aus dem gegebenen Kontext zu antworten und ihn zu zitieren; generieren. Das Abrufen wählt relevante Abschnitte aus, nicht die Antwort. Die Antwort schreibt weiterhin das LLM, jetzt aber auf Basis von Belegen.\n\nTypische Fehler und Gegenmittel: Schlechtes Aufteilen kann eine Schlüsselinformation zerschneiden, vage Fragen holen die falschen Abschnitte. Hybride Suche ergänzt die Vektorähnlichkeit um Schlüsselwortsuche, damit Produktnamen, Modellnummern und Klausel-IDs nicht verloren gehen. Reranking holt zuerst eine größere Menge (etwa die Top 20) und lässt ein zweites Modell jeden Abschnitt genau lesen und neu sortieren. Quellenangaben erlauben Nutzern und Prüfern, das Original nachzulesen.\n\nWann es sich lohnt und wann nicht: RAG rechtfertigt seinen Aufwand, wenn die Antwort in internen Dokumenten steckt, die das Modell nie gesehen hat, wenn sich diese Informationen oft ändern, wenn Antworten nachprüfbare Quellen brauchen oder wenn das Material zu groß für einen einzigen Prompt ist. Überdimensioniert ist es für Allgemeinwissen, das das Modell ohnehin hat, für wenige Seiten, die einfach in den Prompt passen, für strukturierte Daten (Bestellungen, Lagerbestand), bei denen eine SQL-Abfrage oder ein API-Aufruf präziser ist, und für Umformulieren, Übersetzen oder Tonwechsel, bei denen gar nichts abgerufen werden muss. Pilotfit nutzt dieselbe Idee: Beim Abgleich einer Stelle mit den Erfahrungen eines Nutzers zählt die Embedding-Ähnlichkeit zu 60 % und die exakte Übereinstimmung von Tech-Tags zu 40 %, damit eine konkrete Technologie nie allein von der Vektordistanz abhängt.\n\nSo ist die Erklärung gebaut: Eine einzige 3:12-Timeline steuert eine 3D-Szene (Dokumentenstapel, Embedding-Ring, Vektordatenbank-Kugel, Nutzer und LLM-Planet), mit HTML-Overlays für jedes Detailkapitel, damit Text scharf und übersetzbar bleibt. Jedes Kapitel nutzt dasselbe Rückgabe-Beispiel, sodass die Zahlen durchgehend stimmen. Jede Sprache ist eine eigenständige HTML-Datei mit eingebetteten Skripten, Styles und Schriften.',
          features: [
            'Was RAG ist und warum ein LLM ohne RAG halluziniert, gezeigt vor jedem Fachbegriff',
            'Wann sich RAG lohnt und wann es mit Kanonen auf Spatzen schießt',
            'Sechs Kapitel: Vorbereiten → Einbetten → Fragen → Abrufen → Anreichern → Generieren',
            'Aufteilen im Detail: Absatzgrenzen, Überlappung und Quellenangaben',
            'Semantische Karte und Kosinus-Ranking mit sichtbarem Top-K = 3',
            'Praxis-Schutzmechanismen: hybride Suche, Reranking, Quellenangaben',
    ...
    projects: {
      ...
      data: [
        ...
        },
        {
          ...
          description:
            '用一支 3D 動畫解析 RAG（檢索增強生成）：讓 LLM 根據「你自己的文件」回答，而不是憑訓練時的記憶。一個問題「我們的退款期限是多久？」走完六個步驟，從文件切分到附出處的回答，並對照沒有 RAG 時 LLM 憑記憶亂猜的情況。中英雙語、完全離線可用。',
          problem:
            'RAG 是什麼？大型語言模型只能根據訓練時記住的內容作答。這些知識停在某個截止日期，也從沒看過公司內部的文件；所以問它公司規定時，它不會說「我不知道」，而是給出一個流暢又自信的猜測，也就是幻覺。\n\nRAG（Retrieval-Augmented Generation，檢索增強生成）不需要重新訓練模型就能解決這件事：使用者提問時，系統先從你的文件中「檢索」出最相關的段落，再把這些段落連同問題一起交給模型，讓模型根據這些證據作答，並標出答案的出處。\n\n為什麼難解釋？概念很簡單，術語卻很多：embedding、向量資料庫、top-K、餘弦相似度。大多數解釋不是方框加箭頭的架構圖，就是一堆名詞。利害關係人聽完點頭，卻仍期待模型「本來就知道」公司規定，因為他們從來沒看過答案到底從哪裡來。',
          solution:
            '我把整條流程說成一個故事，只圍繞一個具體問題：「我們的退款期限是多久？」\n\n開場先演示失敗情境：沒有 RAG 時，模型回答「通常是 30 天」，沒有任何出處，是一個聽起來合理的幻覺。接著同一個問題走過六個章節：\n1. 準備知識：公司文件切成段落，前後保留一小段重疊，每段都記下出處。\n2. 向量化：embedding 模型把每個段落變成一組向量，也就是它在語意地圖上的座標。\n3. 提問：問題用同一個模型轉成向量。\n4. 檢索：問題向量和所有段落比較，取最接近的 3 段，並顯示相似度分數。\n5. 增強：問題加上這 3 段，組成一個 prompt。\n6. 生成：模型根據這些段落作答，並引用「退款政策 v3 §2」。\n\n最後以實務上讓檢索更可靠的三道防線收尾：混合搜尋、重新排序、引用來源。',
          techDeepDive:
            'RAG 分成兩個階段。\n\n建立索引（離線進行，文件有變動才重跑）：從 PDF、Wiki、資料庫抽出純文字；依段落切成 300–500 字的段落，前後保留一小段重疊，避免一句話被切斷；每段附上出處（文件、章節）；再用預先訓練好的 embedding 模型把每段轉成向量（範例中是 1,536 個數字），意思相近的內容，向量也會彼此靠近；最後把向量、原文與出處一起存進向量資料庫。\n\n查詢階段（每次提問）：用同一個模型把問題轉成向量；以餘弦相似度為所有段落排序，1 代表意思相同、0 代表無關（資料量大時改用近似最近鄰 ANN 索引，不必逐一比較）；取前 K 段（範例中 K = 3，分數為 0.92／0.81／0.77）；組成增強後的 prompt，要求模型只根據提供的內容作答並註明出處；最後生成答案。檢索挑的是「相關段落」，不是答案本身；答案仍由 LLM 撰寫，只是這次有了根據。\n\n常見出錯點與對策：切分不當會把關鍵事實切斷，問題太模糊會撈到錯的段落。混合搜尋在向量相似度之外加上關鍵字比對，產品名稱、型號、條款編號才不會漏掉；重新排序先撈較多候選（例如前 20 段），再讓第二個模型逐段細讀、重新排名；引用來源讓使用者與審核者可以回頭核對原文。\n\n什麼時候該用、什麼時候不用：當答案藏在模型沒看過的內部文件裡、資料經常更新、答案需要附出處讓人核對，或資料量大到一次 prompt 塞不下時，RAG 才值得它帶來的複雜度。反過來，一般常識模型本來就答得出來；只有幾頁的資料直接放進 prompt 就好；訂單、庫存這類結構化資料用 SQL 或 API 查詢更精準；改寫、翻譯、換語氣則完全不需要檢索。這些情況硬上 RAG，就是殺雞用牛刀。Pilotfit 也是同樣的思路：比對職缺和使用者的經歷時，分數 60% 來自 embedding 相似度、40% 來自技術標籤的精確比對，具體的技術名詞不會只靠向量距離決定。\n\n動畫怎麼做：一條 3:12 的時間軸驅動整個 3D 場景（文件堆、embedding 圓環、向量資料庫球體、使用者與 LLM 星球），每個特寫章節以 HTML 疊層呈現，文字清晰且方便翻譯。所有章節都沿用同一份退款政策範例，數字前後一致。每種語言各是一個自包含的 HTML 檔，腳本、樣式與字型全部內嵌。',
          features: [
            '先講清楚 RAG 是什麼、沒有它 LLM 為什麼會幻覺，再進入術語',
            '什麼時候值得用 RAG、什麼時候是殺雞用牛刀',
            '六章流程：準備 → 向量化 → 提問 → 檢索 → 增強 → 生成',
            '切分特寫：依段落切、前後重疊、每段附出處',
            '語意地圖與餘弦相似度排名，看得見 top-K = 3',
            '實務防線：混合搜尋、重新排序、引用來源',
